...
Chapter~\ref{SI} lead to the conclusion that the estimates in metres are
...
abline(v = -T, lty = 2) 
@
\caption{Approximated conditional distribution of the difference of mean
...
$p$-value reported by the $t$-test in 
Chapter~2.
...
generalisation of McNemar's test, 
see Chapter~2)
needs to be applied: %%'
\newpage
...
@
In our case, both versions coincide and one can conclude that the
