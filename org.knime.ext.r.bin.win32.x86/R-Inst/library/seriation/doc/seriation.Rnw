...
by~\cite{seriation:Chen:2002}.  Note that some methods implemented
...
<<label=pimage1, fig=TRUE, include=FALSE>>=
pimage(d, main = "Random")
@
<<label=pimage1-2, fig=TRUE, include=FALSE>>=
pimage(d, o, main = "Reordered")
@
\begin{figure}
    \centering
    \includegraphics[width=6cm]{seriation-pimage1}
    \includegraphics[width=6cm]{seriation-pimage1-2}
...
for the columns (which leaves them in original order) 
to the permutations object using the combine
function \func{c}. This new permutation object for $2$-mode data is used
for displaying the reordered data. The two plots are shown in 
Figure~\ref{fig:pimage2}.

<<label=pimage2, fig=TRUE, include=FALSE>>=
pimage(x, main = "Random")
@

<<label=pimage2-2, fig=TRUE, include=FALSE>>=
...
\begin{figure}
    \centering
    \includegraphics[width=6cm]{seriation-pimage2}
    \includegraphics[width=6cm]{seriation-pimage2-2}
...
\caption{Execution time of seriation of the iris data set for different
    methods (1.5~GHz ix86 system running Ubuntu Linux).}
    \label{tab:timings}
\end{table}
...
(see GW and OLO in Figure~\ref{fig:pimage3}). To investigate this effect,
...
which maintains the original order of the objects, i.e., which returns 
the identity order.
...
it for both data types under the short name `identity'.
