...
/**
 * A {@link LinkedBlockingQueue} which contains {@link RCommand} and as means of starting a thread which will
 * sequentially execute the RCommands in the queue.
 *
...
public class RCommandQueue extends LinkedBlockingQueue<RCommand> {
    ...
    /**
     * Inserts the specified script at the tail of this queue, waiting if necessary for space to become available.
     *
     * @param rScript Script to enqueue for execution
     * @param showInConsole If the command should be copied into the R console.
     * @return {@link RCommand} which wraps the added <code>rScript</code>. Use {@link RCommand#get()} to wait for
     *         execution and fetch the result of evaluation if any.
     */
    public RCommand putRScript(final String rScript, final boolean showInConsole) {
        final RCommand rCommand = new RCommand(rScript.trim(), showInConsole);
    ...
    public interface RCommandExecutionListener {
        ...
        /**
         * Called after a command has been completed, even if an error occurred and
         * {@link #onCommandExecutionError(RException)} was called before.
         *
    ...
    protected class RCommandConsumer extends Thread {
        ...
        private final ExecutionMonitorFactory m_execMonitorFactory;
        ...
        public void run() {
            ...
            try {
                ...
                while (!isInterrupted()) {
                    ...
                    REXP ret = null;
                    for (final RCommand rCmd : commands) {
                        m_listeners.stream().forEach((l) -> l.onCommandExecutionStart(rCmd));

                        final ExecutionMonitor sub = progress.createSubProgress(progressIncrement);
                        sub.setProgress(0.0);
                        if (rCmd.isShowInConsole()) {
                            ...
                            try {
                                ...
                                m_listeners.stream().forEach((l) -> l
                                    .onCommandExecutionError(new RException("Could not execute internal command.", e)));
                            }
