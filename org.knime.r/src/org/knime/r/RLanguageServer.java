 ...
 * <p>
 * The server is started via {@code Rscript --no-save --no-restore --slave -e "languageserver::run()"}. The user is
 * responsible for having R and the {@code languageserver} package installed. Configure the R executable path in the
 * KNIME preferences (Preferences → KNIME → R).
 *
 * @author KNIME GmbH
 */
...
final class RLanguageServer {
    ...
    static LanguageServerProxy startLanguageServer() throws IOException {
        ...
        LOGGER.debug("R LSP: Rscript path from KNIME preferences: " + rscriptPath);
        ...
        // Pre-flight: verify that the languageserver package is installed before starting the server process,
        // so we get a clear error message instead of a silent crash.
        LOGGER.debug("R LSP: Checking that the 'languageserver' package is installed...");
        try {
            final Process check = new ProcessBuilder(rscriptPath, "--no-save", "--no-restore", "--slave", //
                "-e", "if (!requireNamespace('languageserver', quietly=TRUE)) stop('languageserver not installed')") //
                    .start();
            final String stderr =
                new String(check.getErrorStream().readAllBytes(), StandardCharsets.UTF_8).trim();
        ...
        // Diagnostics (lintr) are disabled because the scripting editor uses a virtual inmemory:// document URI,
        // and lintr's normalizePath() fails on non-file URIs.
        final String rExpr = "options(languageserver.diagnostics = FALSE); languageserver::run()";
        LOGGER.debug("R LSP: Starting language server with: " + rscriptPath + " -e \"" + rExpr + "\"");

        final ProcessBuilder pb = new ProcessBuilder(rscriptPath, "--no-save", "--no-restore", "--slave", //
            "-e", rExpr);

        // Set working directory to the user's home so that callr (used internally by languageserver for completion
        // and parsing) can create temp files and spawn subprocesses successfully.
        pb.directory(Path.of(System.getProperty("user.home")).toFile());

        // Ensure R's bin directory is on PATH so that callr/processx can find R.exe when spawning background
        // subprocesses for completions.
        ...
        return new LanguageServerProxy(pb);
    }
}
