...
dangerous to rely on asymptotic tests. As shown in 
Chapter~3,
conditioning on the data and computing the distribution of the test
statistics without additional assumptions is one alternative. The function
...
survival times differ for grade III patients:
...
or \stress{blocking}, with respect tumor grading:
...
\begin{figure}
\begin{center}
<<SA-GBSG2-plot, echo = TRUE, fig = TRUE>>=
...
in 
Chapter 5
for linear regression model are not available,
...
responses in 
Chapter~8
are applicable to censored responses in 
...
the conditional trees of the latter selects cutpoints based on 
log-rank statistics;
...
this corresponds to the $p$-value associated with this variable in Cox's %%'s
...
plot(GBSG2_ctree)
@
\caption{\Robject{GBSG2} data: Conditonal inference tree with the survival function,
